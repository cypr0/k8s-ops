...
- **The playbook provisions an `ansible` OS user with a dedicated pubkey and passwordless sudo, but the CronJob's own inventory connects as `root`** (`ansible_user=root` in `run.sh`'s generated `inventory.ini`). The `ansible` user/`PROXMOX_ANSIBLE_SSH_PUBKEY` setup exists but isn't actually used by this playbook's own runs — worth clarifying its intended purpose (see TODOs).
...
## TODOs / unknowns
- Why the `ansible` OS user + pubkey are provisioned when the CronJob's own inventory connects as `root` — not verified from the repo whether this is for a separate manual/human access path or leftover from an earlier design.
