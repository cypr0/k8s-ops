...
- **Depended on by:** `paperless-ngx`'s IMAP mail-fetch integration (port 993, in-cluster via `mail.${SECRET_DOMAIN}`), and `mailu-mcp` (`kubernetes/apps/hermes-agent/mailu-mcp/`) for the same IMAP access plus CalDAV (`webdav`/Radicale) on behalf of hermes-agent — see `docs/apps/mailu-mcp.md`. Authentik's own outbound mail (`kubernetes/apps/security/authentik/app/helmrelease.yaml`'s `authentik.email.host: ${SECRET_MAIL_SERVER}`) points at a *different*, pre-existing mail server, not at this Mailu instance — same as it did with Stalwart.
