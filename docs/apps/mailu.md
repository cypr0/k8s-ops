...
| `kubernetes/apps/mail/mailu/app/ciliumnetworkpolicy.yaml` | Ingress from `world` on the enabled mail ports, from the `cloudflare-tunnel` pod (namespace `network`) on `443`, from `192.168.10.0/24`/`192.168.40.0/24` (LAN) also on `443`; same-namespace trust for the chart's internal component-to-component traffic; egress to Postgres, DNS, and `world` (outbound SMTP delivery + ClamAV virus-DB updates) |
