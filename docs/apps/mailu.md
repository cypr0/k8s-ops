...
| `kubernetes/apps/mail/mailu/app/service-front-webmail-lan.yaml` | LAN-only path to `webmail.${SECRET_DOMAIN}` — second `LoadBalancer` Service sharing `mailu-front-ext`'s IP, carries the `coredns.io/hostname` annotation `k8s-gateway` needs to generate an internal DNS record |
| `kubernetes/apps/mail/mailu/app/ciliumnetworkpolicy.yaml` | Ingress from `world` on the enabled mail ports, from the `cloudflare-tunnel` pod (namespace `network`) on `443`, from `192.168.10.0/24` and `10.10.0.2/32` (LAN) also on `443`; same-namespace trust for the chart's internal component-to-component traffic; egress to Postgres, DNS, and `world` (outbound SMTP delivery + ClamAV virus-DB updates) |
...
- **Webmail/admin UI (443/HTTPS) — two parallel paths**, both terminating at the same `front` pod, on a *separate* hostname (`webmail.${SECRET_DOMAIN}`):
  ...
  - **From the home/office LAN: directly**, via `service-front-webmail-lan.yaml` — a second `LoadBalancer` Service sharing `mailu-front-ext`'s IP (`192.168.10.104`, via a matching `io.cilium/lb-ipam-sharing-key` annotation on both — Cilium's LB-IPAM otherwise refuses to hand the same IP to a second Service) and carrying the `coredns.io/hostname: webmail.${SECRET_DOMAIN}` annotation so `k8s-gateway` (the in-cluster authoritative resolver for `${SECRET_DOMAIN}`, used by the LAN's split-DNS setup) generates an internal record for it — the way it already does for `mail.${SECRET_DOMAIN}`. Without this, LAN clients get `NXDOMAIN` for `webmail.${SECRET_DOMAIN}`: it otherwise has no Gateway API `HTTPRoute` or annotated Service anywhere for `k8s-gateway` to derive a record from. `ciliumnetworkpolicy.yaml` restricts this path's ingress to `192.168.10.0/24` (the Proxmox/VM LAN this cluster's nodes live on) and `10.10.0.2/32` — not `world`, since nothing else stops that IP from being reachable from anywhere routable to `192.168.10.0/24`. That second address is **not** the office LAN's own `192.168.40.0/24` range: OPNsense masquerades office-LAN clients' traffic to the WireGuard tunnel interface's own address before it enters the site-to-site tunnel into the Proxmox network, rather than routing it through with the original source preserved — confirmed live via `cilium monitor` (connections arrived as `10.10.0.2`, and were dropped until this was added). If OPNsense's WireGuard setup ever changes, this CIDR needs updating to match.
