...
- **Depends on:**
  ...
  - Mailu's IMAP service (`mail.${SECRET_DOMAIN}`, `kubernetes/apps/mail/mailu/`), in-cluster since the Netcup mailbox migration — `ciliumnetworkpolicy.yaml`'s egress rule is a `toEndpoints` match on Mailu's `front` pods, not a `toFQDNs` rule, since this is now a same-cluster call.
...
- **The IMAP mail-fetch egress rule exists, but no matching env var does.** `ciliumnetworkpolicy.yaml` allowlists Mailu's `front` pods on `993/TCP`, but no `PAPERLESS_EMAIL_*` config appears anywhere in `helmrelease.yaml` — the actual Mail Account/Mail Rule is paperless-ngx's own DB-backed config, set via its admin GUI, not tracked in this repo. Its server field must stay `mail.${SECRET_DOMAIN}` (not the internal Service DNS name) so TLS hostname verification against Mailu's real certificate still succeeds — see the CoreDNS split-horizon comment in `kubernetes/apps/kube-system/coredns/app/helmrelease.yaml` for why that hostname still resolves correctly in-cluster.
