...
> **Hostname**   `*.${SECRET_DOMAIN}`, `external.${SECRET_DOMAIN}`, `philipp-rosch.de`, `www.philipp-rosch.de`
...
## Routing & access
- No HTTPRoute of its own — routing is defined entirely by the `config.yaml` ConfigMap mounted into the pod: every hostname under `*.${SECRET_DOMAIN}`, plus the separate apex domain `philipp-rosch.de`/`www.philipp-rosch.de` (a different zone, same Cloudflare account/tunnel — see the inline comment), is forwarded to `envoy-external`; anything unmatched falls through to `service: http_status:404`. `originServerName` is pinned to `external.${SECRET_DOMAIN}` for all of them — that's just the TLS SNI cloudflared presents to Envoy's existing wildcard cert; Envoy still routes by the real HTTP `Host` header regardless.
