...
- **Depended on by:** End-users via mobile apps and web UI, plus two API consumers added later: Open WebUI's `immich_photos` Tool (`kubernetes/apps/open-webui/open-webui/app/tools/immich_photos.py`) and the `immich-mcp` MCP server that serves a byte-identical mirror of it to `hermes-agent` (`kubernetes/apps/hermes-agent/immich-mcp/`). Both authenticate with an **Immich API key that has to be minted by hand** — see `docs/apps/immich-mcp.md`; there is no service-account concept here, and this instance is OIDC-only with `passwordLogin.enabled: false`, so nothing can provision one automatically. Both are read-mostly and refuse `DELETE` outright.
