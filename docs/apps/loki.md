...
Loki is now the cluster's **only** log store. It used to run alongside OpenSearch, with Fluent Bit dual-writing syslog, Suricata and Talos streams to both; OpenSearch was removed on 2026-09-20 (see `docs/apps/fluent-bit.md` for what moved where). Two writers remain: Alloy for all pod logs and Kubernetes events, and Fluent Bit for Talos node logs. Security events no longer land here at all — OPNsense, Suricata and Proxmox report to Wazuh instead (`docs/apps/wazuh.md`).
