...
| `kubernetes/apps/hermes-agent/nextcloud-mcp/app/nextcloud_full.py` | The actual Nextcloud tool implementation — 22 methods incl. `capabilities()`, `raw_ocs_request()` and `raw_webdav_request()` (mirrored from Open WebUI's copy) |
...
- **The tool was cut from 85 methods to 22 in v2.0.0, without losing reach.** Every method's signature and docstring is compiled into a JSON schema and injected into the consuming model's context, so 85 here plus 139 in `paperless-mcp` meant hermes-agent facing **232** MCP tools. The v2 shape keeps the file, trash, version, share and directory-read operations typed with their v1 names and parameters unchanged, and pushes the long tail — favorites, comments, systemtags, federated shares, user/group mutation, subadmins, apps, notifications, activity, user status, preferences, out-of-office, recommendations, server capabilities — behind `raw_ocs_request()` / `raw_webdav_request()`, which were already the documented escape hatches in v1. Measured with Open WebUI's own `get_tool_specs()`: ~6,200 → ~2,300 tokens. Full reasoning in `docs/apps/open-webui.md`.
- **v1's `get_capabilities()` is gone on purpose rather than kept.** A `get_capabilities()` (Nextcloud server feature flags) sitting next to the new `capabilities()` (this tool's own on-demand cheat sheet for driving the escape hatches) is precisely the near-name collision that made hermes-agent's Paperless prompt need a disambiguation paragraph. Server capabilities now live at `capabilities("endpoints")` as a documented `raw_ocs_request` path.
